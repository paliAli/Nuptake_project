...
#   group_by(Date)%>%
#   summarise(mean_LAI = first(mean_LAI), meanNDVI = first(meanNDVI), meanNDRE = first(meanNDRE), meanMCARI = first(meanMCARI))
...
ggplot(VI_long[which(VI_long$date >= "2021-01-24"),], aes(date, values, color = vegetation_index)) + 
...
index_colors <- c("purple", "gold", "brown2", "chartreuse2", "dodgerblue")

# Define custom labels for legend
index_labels <- c( mean_field_height = "Crop height", mean_LAI = "LAI", meanNDVI = "NDVI", meanNDRE = "NDRE", meanMCARI = "MCARI")
